...
//    }
    ...
    private void init() {
        ...
        if (!mAlreadyGetStatusBarHeight) {
            ...
            if (resourceId > 0) {
                ...
                LogUtils.d(TAG, String.format("Get status bar height %d", mStatusBarHeight));
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        do {
            ...
            if (bottom == null) {
                LogUtils.d(TAG, "bottom == null break;");
                break;
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (Math.abs(maxBottom - b) == mStatusBarHeight) {
            LogUtils.w(TAG, String.format("customRootLayout on layout get max bottom value offset just equal statusBar height %d", mStatusBarHeight));
            return;
        }

        if (b >= maxBottom && maxBottom != 0) {
            // 在底部，键盘隐藏状态
            LogUtils.d(TAG, "keyboard hiding");
            onKeyboardShowing(false);
        } else if (maxBottom != 0) {
            LogUtils.d(TAG, "keyboard showing");
    ...
    public void onGlobalLayout() {
        ...
        if (keyboardHeight == mStatusBarHeight) {
            LogUtils.w(TAG, String.format("On global layout change get keyboard height just equal statusBar height %d", keyboardHeight));
            return;
        }

        mLastHeight = height;

        final boolean change = KeyBoardUtils.saveKeyboardHeight(getContext(), keyboardHeight);
        if (change) {
            final int panelHeight = getPanelLayout(this).getHeight();
            final int validPanelHeight = KeyBoardUtils.getValidPanelHeight(getContext());
