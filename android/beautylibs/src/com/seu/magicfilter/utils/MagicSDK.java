...
public class MagicSDK {
    ...
    public void onStartSkinSmooth(float level) {
        if (_bitmapHandler == null)
            return;
        if (level > 10 || level < 0) {
            Logger.e("MagicSDK", "Skin Smooth level must in [0,10]");
            return;
        }
    ...
    public void onStartWhiteSkin(float level) {
        if (_bitmapHandler == null)
            return;
        if (level > 5 || level < 0) {
            Logger.e("MagicSDK", "Skin white level must in [1,5]");
            return;
        }
    ...
    public void initMagicBeauty() {
        if (_bitmapHandler == null) {
            Logger.e("MagicSDK", "please storeBitmap first!!");
            return;
        }
    ...
    protected void finalize() throws Throwable {
        ...
        if (_bitmapHandler == null)
            return;
        Logger.w("MagicSDK", "JNI bitmap wasn't freed nicely.please remember to free the bitmap as soon as you can");
        freeBitmap();
        Logger.w("MagicSDK", "MagicSDK wasn't uninit nicely.please remember to uninit");
        uninitMagicBeauty();
    }
