			...
			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					if (mode.showHeaderLoadingLayout()) {
						...
						int temp = (int) (scaleFactor * (currentScrollValue + newScrollValue));
						if(temp<0){
							temp = 0;
						}
						view.setHeaderScroll(temp);
					}
					...
					if (mode.showFooterLoadingLayout()) {
						...
						int temp = (int) (scaleFactor * (currentScrollValue + newScrollValue - scrollRange));
						if(temp<0){
							temp = 0;
						}
						view.setHeaderScroll(temp);
