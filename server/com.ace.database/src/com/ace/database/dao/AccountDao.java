...
public class AccountDao extends BasicDao {
	 ...
	 */
	public HashMap<String, ArrayList<AccountDetail>> getAccountDetailList(String userId, String beginTime, String endTime, Constant.ChangeType type, int page, int pageSize, String logicType) {
		...
		String logicTypesql = "";
		// ORDER("1"), SYS_ADD_MONEY("2"), SEND_GIFT("3"), EXCHANGE("4"), SYS_DEC_MONEY("5");
		// 业务类型
		if (!Validator.isEmpty(logicType)) {
			logicTypesql = " and channel=?";
		}
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			String sql = " select id, userId,userName,relatedUserId,relatedUserName,amount,channel,desciption,createTime from   " + dataBase + " where userId=? and createTime>=? and createTime<=? " + logicTypesql + " order by createTime desc limit ?,? ";
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, userId);
