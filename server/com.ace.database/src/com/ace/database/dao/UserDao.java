...
public class UserDao extends BasicDao {
	...
	public String addUser(User user) {
		...
		try {
			ps = this.getConnnection().prepareStatement("insert into user_info(userId,passwd,sex,nickName,channel,rank,basicScore,signature,profile,showImg,updateTime,createTime,schemaId) values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			ps.setString(11, DateUtil.getTimeInMillis());
	...
	public User getUser(String userId) {
		...
		ResultSet res = null;
		String sql = "select uuid,userId,passwd,sex,nickName,channel,rank,basicScore,signature,profile,showImg,isActive,isAuthen,isForbidShow,updateTime,createTime,schemaId from user_info where userId=?";
		try {
			...
			if (res != null && res.next()) {
				...
				user.setIsAuthen(res.getString("isAuthen"));
				user.setIsForbidShow(res.getString("isForbidShow"));
	...
	public ArrayList<User> getSynUser(String synId, int maxCnt) {
		...
		PreparedStatement ps = null;
		ResultSet res = null;
		String sql = "select uuid,info.userId,mapping.tokenId,passwd,sex,nickName,info.channel,rank,basicScore,signature,profile,showImg,isActive,isAuthen,isForbidShow,updateTime,createTime,schemaId from user_info info inner join user_tokenId_mapping mapping on info.userId=mapping.userId  where schemaId>? order by schemaId  limit ?";
		try {
			ps = this.getConnnection().prepareStatement(sql);
	...
	public String updateStatus(String userId, String isActive, String isAuthen, String isForbidShow) {
		PreparedStatement ps = null;
		String sql = "update user_info set isActive=?,isAuthen=?,isForbidShow=?,schemaId=? where userId=?";
		try {
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, isActive);
			ps.setString(2, isAuthen);
			ps.setString(3, isForbidShow);
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月1日下午8:11:40
	 */
	public List<UserAuth> getUserAuthList(String userId, String status) {
		ArrayList<UserAuth> list = new ArrayList<UserAuth>();
		PreparedStatement ps = null;
		ResultSet res = null;
		StringBuffer sql = new StringBuffer("select  userId, name, phone, cardType, cardId, bankId, bankName, bankProvince, bankCity, bankAddress, frontImg, backImg, status from user_authen_info ");
		if (!Validator.isEmpty(userId) && !Validator.isEmpty(status)) {
			sql.append(" where userId=? and status=? ");
		...
		try {
			...
			while (res != null && res.next()) {
				...
				userAuth.setUserId(res.getString("userId"));
				userAuth.setName(res.getString("name"));
				userAuth.setPhone(res.getString("phone"));
				userAuth.setCardType(res.getString("cardType"));
				userAuth.setCardId(res.getString("cardId"));
				userAuth.setBankId(res.getString("bankId"));
				userAuth.setBankName(res.getString("bankName"));
				userAuth.setBankProvince(res.getString("bankProvince"));
				userAuth.setBankCity(res.getString("bankCity"));
				userAuth.setBankAddress(res.getString("bankAddress"));
				userAuth.setFrontImg(res.getString("frontImg"));
				userAuth.setBackImg(res.getString("backImg"));
				userAuth.setStatus(res.getString("status"));
				list.add(userAuth);
			}
			return list;
		} catch (Exception e) {
			this.getLogger().error("[获取用户审核数据失败]", e);
		} finally {
			DBReleaser.release(ps, res);
		}
		return list;
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月1日下午8:23:22
	 */
	public String addUserAuth(UserAuth userAuth) {

		PreparedStatement ps = null;
		try {
			ps = this.getConnnection().prepareStatement("insert into user_authen_info(userId, name, phone,  cardType, cardId, bankId,bankName, bankProvince, bankCity,bankAddress, frontImg, backImg, status, createTime, updateTime, schemaId,holdImg) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			ps.setString(1, userAuth.getUserId());
			ps.setString(2, userAuth.getName());
			ps.setString(3, userAuth.getPhone());
			ps.setString(4, userAuth.getCardType());
			ps.setString(5, userAuth.getCardId());
			ps.setString(6, userAuth.getBankId());
			ps.setString(7, userAuth.getBankName());
			ps.setString(8, userAuth.getBankProvince());
			ps.setString(9, userAuth.getBankCity());
			ps.setString(10, userAuth.getBankAddress());
			ps.setString(11, userAuth.getFrontImg());
			ps.setString(12, userAuth.getBackImg());
			ps.setString(13, userAuth.getStatus());
			ps.setString(14, DateUtil.getStringDate());
			ps.setString(15, DateUtil.getStringDate());
			ps.setString(16, DateUtil.getTimeInMillis());
			ps.setString(17, userAuth.getHoldImg());
			if (ps.executeUpdate() <= 0) {
				return Constant.OperaterStatus.FAILED.getValue();
			}
			return Constant.OperaterStatus.SUCESSED.getValue();
			...
			e.printStackTrace();
			this.getLogger().error("[添加 userID[" + userAuth.getUserId() + "}用户审核信息失败]", e);
		} finally {
			DBReleaser.release(ps);
		}
		return Constant.OperaterStatus.SUCESSED.getValue();
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月1日下午8:43:16
	 */
	public String updateUserIsAuthen(String userId, String status) {
		return updateUserAuthStatus(userId, status);
	}
	...
	public String updateUserAuthStatus(String userId, String status) {
		PreparedStatement ps = null;
		String sql = "update user_info set isAuthen=?,schemaId=?  where userId=?";
		try {
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, status);
			ps.setString(2, DateUtil.getTimeInMillis());
			...
			this.getLogger().error("[更新userId[" + userId + "];Auth[" + status + "]用户的审核状态失败]", e);
		} finally {
			DBReleaser.release(ps);
		}
		return Constant.OperaterStatus.FAILED.getValue();
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月2日下午5:11:25
	 */
	public String updateUserAuth(UserAuth userAuth) {

		PreparedStatement ps = null;
		try {
			ps = this.getConnnection().prepareStatement("update user_authen_info set name=?, phone=?, cardType=?," + " cardId=?, bankId=?, bankName=?, bankProvince=?, bankCity=?," + " bankAddress=?, frontImg=?, backImg=?, status=?, updateTime=?, schemaId=?, holdImg=? where userId=?");
			ps.setString(1, userAuth.getName());
			ps.setString(2, userAuth.getPhone());
			ps.setString(3, userAuth.getCardType());
			ps.setString(4, userAuth.getCardId());
			ps.setString(5, userAuth.getBankId());
			ps.setString(6, userAuth.getBankName());
			ps.setString(7, userAuth.getBankProvince());
			ps.setString(8, userAuth.getBankCity());
			ps.setString(9, userAuth.getBankAddress());
			ps.setString(10, userAuth.getFrontImg());
			ps.setString(11, userAuth.getBackImg());
			ps.setString(12, userAuth.getStatus());
			ps.setString(13, DateUtil.getStringDate());
			ps.setString(14, DateUtil.getTimeInMillis());
			ps.setString(15, userAuth.getHoldImg());
			ps.setString(16, userAuth.getUserId());
			if (ps.executeUpdate() <= 0) {
				return Constant.OperaterStatus.FAILED.getValue();
			}
			return Constant.OperaterStatus.SUCESSED.getValue();
			...
			e.printStackTrace();
			this.getLogger().error("[修改 userID[" + userAuth.getUserId() + "}用户审核信息失败]", e);
		} finally {
			DBReleaser.release(ps);
		}
		return Constant.OperaterStatus.SUCESSED.getValue();
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月13日下午2:02:25
	 */
	public boolean isManager(String userId) {
		PreparedStatement ps = null;
		ResultSet res = null;
		boolean exeCode = false;
		String sql = "   select count(1) as num from user_admin_privilege where userId = ? LIMIT 0,1";
		try {
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, userId);
			res = ps.executeQuery();
			if (res != null && res.next() && res.getInt("num") > 0) {
				exeCode = true;
			}
		} catch (Exception e) {
			this.getLogger().error("[获取用户及设备信息失败]", e);
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月13日下午2:38:07
	 */
	public ArrayList<String> getUserMenu(String userId) {
		PreparedStatement ps = null;
		ResultSet res = null;
		ArrayList<String> list = new ArrayList<String>();
		String sql = "select menuid from user_admin_privilege where userId = ?  order by menuid ";
		try {
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, userId);
			res = ps.executeQuery();
			while (res != null && res.next()) {
				list.add(res.getString("menuid"));
			}
		} catch (Exception e) {
			this.getLogger().error("[获取用户及设备信息失败]", e);
		} finally {
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since 2016年6月13日下午4:11:25
	 */
	public String updateUserMenu(String userId, String menus) {
		PreparedStatement ps = null;
		String sql = "delete from user_admin_privilege where userId=?";
		try {
			ps = this.getConnnection().prepareStatement(sql);
			ps.setString(1, userId);
			if (ps.executeUpdate() < 0) {
			...
			if (!Validator.isEmpty(menus)) {
				...
				sql = "insert into user_admin_privilege(userId,menuId) VALUES(?,?)";
				ps = this.getConnnection().prepareStatement(sql);
				for (int i = 0; i < menuid.length; i++) {
					ps.setString(1, userId);
					ps.setString(2, menuid[i]);
					ps.addBatch();
				}
				ps.executeBatch();
			...
			this.getLogger().error("[更新userId[" + userId + "]", e);
		} finally {
			DBReleaser.release(ps);
		}
		return Constant.OperaterStatus.FAILED.getValue();
