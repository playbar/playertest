...
public class GiftDao extends BasicDao {
	...
	public ArrayList<GiftDetail> getSynReceiveGiftDetailList(String synId, int maxCnt){
		...
		ResultSet rs = null;
		try {
			String sql = " select id, userId,userName,relatedUserId,relatedUserName,giftId,giftName,giftCnt,price,totalValue,channel,description,createTime from receive_detail where id>? order by id limit ?";
			ps = this.getConnnection().prepareStatement(sql);
			ps.setInt(1, Integer.valueOf(synId));
			ps.setInt(2, maxCnt);
			rs = ps.executeQuery();
			...
			this.getLogger().error("[查询收到的礼物明细失败]", e);
		} finally {
			DBReleaser.release(ps, rs);
		}
		return list;
