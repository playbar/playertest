...
public class AccountService {
	 ...
	 */
	public static HashMap<String, ArrayList<AccountDetail>> getAccountDetail(String userId, Constant.ChangeType type, String beginTime, String endTime, int page, int pageSize, String logicType) {
		UserTransaction ut = UserTransactionManager.getUserTransaction();
		HashMap<String, ArrayList<AccountDetail>> val = null;
		AccountModule am = new AccountModule();
		try {
			ut.begin();
			val = am.getAccountDetailList(userId, type, beginTime, endTime, page, pageSize,logicType);
			ut.commit();
			return val;
	...
	public static String updateBalance(String userId, String relateUserId, String amount) {
		...
		try {
			...
			if(num.longValue() < 0){
				...
				exeRes = am.reduceMoney(userId, relateUserId, num.multiply(new BigDecimal(-1)), type);
			} else {
	 ...
	 * @return
	 * @author ZuKang.Song
	 * @since  2016年6月22日下午2:33:17
	 */
	public static BigDecimal getIncomeBalance(String userId) {
		UserTransaction ut = UserTransactionManager.getUserTransaction();
		AccountModule am = new AccountModule();
		try {
			ut.begin();
			BigDecimal balacne = am.getIncome(userId);
			ut.commit();
			return balacne;

		} catch (Exception e) {
			try {
				logger.error("[获取" + userId + "A豆余额失败]", e);
				ut.rollback();
			} catch (Exception e1) {
				e1.printStackTrace();
			}
