...
public class OrderHandler extends BasicServiceAdapter {
	...
	private void applePayCallBack() {
		...
		Order order = OrderService.getOrderWithThirdId(ticket);
		...
		JSONObject jo = null;
		try {
			jo = this.verifyApplePay(payUrl, ticket);
			this.getLog().info("[apple pay]" + jo.toJSONString());
			if (jo == null || jo.getInteger("status").intValue() != 0) {
				this.setError("713");
			}
			JSONArray goods = jo.getJSONObject("receipt").getJSONArray("in_app");
			...
			// 创建订单
			if (order == null) {
				orderId = OrderService.createOrder(userId, ticket, userId, gi.getId(), Integer.parseInt(cnt), payChannel.APPLE_INNEL);
			} else {
	...
	public static void main(String[] args) {
		String str = "orderInfo:partner=\"2088801770882180\"&seller_id=\"yejinqiang@funfun001.com\"&out_trade_no=\"20160310053441740088037790\"&subject=\"5000元充值\"&body=\"1500元充值\"&total_fee=\"1500\"&notify_url=\"http://172.16.150.2/serveice\"&service=\"mobile.securitypay.pay\"&payment_type=\"1\"&_input_charset=\"utf-8\"&it_b_pay=\"30m\"&return_url=\"m.alipay.com\"";
