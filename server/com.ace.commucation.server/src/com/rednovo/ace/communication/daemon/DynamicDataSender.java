...
//
//					logger.info("-----------------------------------------");
//					logger.info("动态数据推送：" + showId);
//					long realMan = LiveShowManager.getMemberCnt(showId);
//					long robotMan = LiveShowManager.getRobotCnt(showId);
//					long total = (LiveShowManager.getMemberCnt(showId) - 1) * 14 + LiveShowManager.getRobotCnt(showId);
//					logger.info("realMan：" + realMan + ",robotMan:" + robotMan + ",total:" + total);
//					logger.info("-----------------------------------------");

					String memberCnt = String.valueOf((LiveShowManager.getMemberCnt(showId) - 1) * 14 + LiveShowManager.getRobotCnt(showId));

					String newSuptCnt = LiveShowManager.getNewSupportCnt(showId);
					String totalSupCnt = LiveShowManager.getTotalSupportCnt(showId);
					String income = UserManager.getIncome(showId);
...
//					logger.info("[推送房间" + showId + "动态数据][总点赞数:" + totalSupCnt + ",新点赞" + newSuptCnt + ",观众" + memberCnt + "]");
					this.sendMsg(showId, newSuptCnt, totalSupCnt, memberCnt, income);
				}
	...
	private void sendMsg(String showId, String newSuptCnt, String totalSuptCnt, String memberCnt, String income) {
