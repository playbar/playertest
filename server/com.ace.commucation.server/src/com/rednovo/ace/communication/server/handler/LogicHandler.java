...
public class LogicHandler {
	...
	public LogicHandler(String sessionId, Message msg) {
		...
		if (Constant.MsgType.TXT_MSG.getValue().equals(msg.getSumy().getMsgType())) {
			this.bd = JSON.parseObject(this.requestBody.getString("basicData"));
		}
	...
	public Message enter() {
		...
		logger.info("[LoigcHandler][" + userId + ",进入房间" + showId + "]");
		...
		if (!Validator.isEmpty(userId)) {
			...
			if (u.getIsActive().equals(userStatus.FREEZE.getValue())) {
				this.failed("208", "用户已经被冻结");
				return this.responseMsg;
			}
			if (userId.equals(showId) && userStatus.FORBIDSHOW.getValue().equals(u.getIsForbidShow())) {// 主播进入房间开播
				this.failed("222", "用户被禁播");
				return this.responseMsg;
			}
		}
		...
		if (Validator.isEmpty(userId)) {
			...
			jo.put("channel", u.getChannel());// 用户来源，用以辨识机器人，便于客户端筛选机器人消息
	...
	public Message sendGift() {
		...
		if (Constant.OperaterStatus.SUCESSED.getValue().equals(response.getString("exeStatus"))) {
			...
			Gift g = StaticDataManager.getGift(giftId);

			// TODO 添加参数 newGift,兼容老版本连送机制
			String newGift = this.requestBody.getString("newGift");

			if ("1".equals(newGift) && "0".equals(g.getType())) {// 连送礼物
				this.requestBody.put("cnt", LiveShowManager.addSeriesGift(senderId, showId, giftId));
			}
	 ...
	 * @return
	 * @author Yongchao.Yang
	 * @since 2016年6月1日下午12:59:30
	 */
	public Message switchNet() {
		if (!this.validateKey("002-014")) {
			return this.responseMsg;
		}
		String showId = this.requestBody.getString("showId");
		...
		// 广播网络
		this.requestBody.put("showId", showId);
		MessageSender.sendPublicMessage(this.requestMsg, showId);
	...
	public Message exit() {
		...
		if (showId.equals(UserId) || isForce) {
			...
			this.setValue("memberCnt", String.valueOf(Long.parseLong(data.get("MEMBER")) + LiveShowManager.getRobotCnt(showId)));
	...
	public Message chat() {
		...
		// 判断用户是否禁言
		String userId = this.requestMsg.getSumy().getSenderId();
		String showId = this.requestMsg.getSumy().getShowId();

		User u = UserManager.getUser(userId);
		if (u.getIsActive().equals(userStatus.FREEZE.getValue())) {
			this.failed("208", "您已经被封禁");
			return this.responseMsg;
		}

		if (LiveShowManager.isUserForbidden(userId, showId)) {
			this.failed("221", "您已经被禁言");
			return this.responseMsg;
		}
