...
build()
{
	...
	XCFLAGS="-O0 -isysroot ${SDK} -arch $ARCH -miphoneos-version-min=${SDK_VERSION_MIN} -fembed-bitcode" \
	XLDFLAGS="-isysroot ${SDK} -arch $ARCH -miphoneos-version-min=${SDK_VERSION_MIN} " \
	make SYS=darwin
