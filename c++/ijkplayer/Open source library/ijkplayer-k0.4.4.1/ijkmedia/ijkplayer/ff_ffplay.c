...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
   ...
   // __android_log_print(ANDROID_LOG_INFO,"jnilog","quepic srcwidth=%d, srcheight=%d, vpwidth=%d, vpheight=%d", \
        src_frame->width, src_frame->height, vp->width, vp->height);
...
{
    __android_log_print(ANDROID_LOG_INFO,"jnilog", "configure_video_filters() enter");
    __android_log_print(ANDROID_LOG_INFO,"jnilog", "vfilters=%s", vfilters);
...
#define INSERT_FILT(name, arg) do {                                          \
    __android_log_print(ANDROID_LOG_INFO,"jnilog", "INSERT_FILT name=%s arg=%s", name, arg); \
    ...
    int my_crop_width = 0;
    int my_crop_height = 0;
    if (my_calc_crop_size(frame->width, frame->height, &my_crop_width, &my_crop_height) < 0)
    {
       // __android_log_print(ANDROID_LOG_INFO,"jnilog", "calc_crop_size() fail \
            in_width=%d in_height=%d, crop_width=%d crop_height=%d, out_width=%d out_height=%d", \
            frame->width, frame->height, my_crop_width, my_crop_height, my_out_width, my_out_height);       
        goto fail;  
    }
    ...
    char my_args[128] = {0};
    sprintf(my_args, "floor((in_w-%d)/2)*2:floor((in_h-%d)/2)*2", frame->width - my_crop_width, frame->height - my_crop_height);


    INSERT_FILT("crop", my_args);
...
{
    __android_log_print(ANDROID_LOG_INFO,"jnilog", "ffplay_video_thread enter");
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
    AVFrame *frame = av_frame_alloc();
    double pts;
...
#if CONFIG_AVFILTER
    __android_log_print(ANDROID_LOG_INFO,"jnilog", "avfilter_graph_alloc()");
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            graph = avfilter_graph_alloc();
             ...
             __android_log_print(ANDROID_LOG_INFO,"jnilog", "ffp->vfilters_list=0x%x is->vfilter_idx=%d", 
...
static int read_thread(void *arg)
{
	...
	__android_log_print(ANDROID_LOG_INFO,"jnilog","avctxwidth=%d, avctxheight=%d",avctx->width, avctx->height);
        int my_crop_width = 0;
        int my_crop_height = 0;
        my_calc_crop_size(avctx->width, avctx->height, &my_crop_width, &my_crop_height);
        ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, my_crop_width, my_crop_height);
