...
// 打开推流
bool CPushStreamEngine::Open(CPushStreamNotify* pNotify)
{
	av_register_all();
    ...
    //创建视频编码器
    if(m_PushStreamParam.bHardEncode)
    {
#if TARGET_OS_IPHONE
        m_pVideoEncode = new CVideoEncodeVt();
#endif
    }
    if(NULL == m_pVideoEncode)
    {
        m_pVideoEncode = new CVideoEncode();
        if(NULL == m_pVideoEncode)
        {
            CLog::GetInstance().Log(ENUM_LOG_LEVEL::enum_Log_Level5, "create VideoEncode Failed!");
            assert(false);
            return false;
        }
    }
	...
	// 打开视频编码
	if (!m_pVideoEncode->Open(m_PushStreamParam))
	{
...
void CPushStreamEngine::SetHardEncode(bool abUse)
{
    m_PushStreamParam.bHardEncode = abUse;
}
	...
	while (!m_bVideoEncodeThreadExit)
	{
		try
		{
            ...
            else
            {
                if(bCodecClose)
                {
                    int iNetworkStatus = m_PushStreamStatus.GetNetworkStatus();
                    if(iNetworkStatus == ENUM_PUSH_STREAM_NETWORK_STATUS_GOOD)
                    {
                        m_PushStreamParam.iVideoBitRate = 600000;
                        m_PushStreamParam.iVideoMinBitRate = 600000;
                        m_PushStreamParam.iVideoMaxBitRate = 800000;
                        m_pVideoEncode->Open(m_PushStreamParam);
                        
                    }
                    else if(iNetworkStatus == ENUM_PUSH_STREAM_NETWORK_STATUS_BAD)
                    {
                        m_PushStreamParam.iVideoBitRate = 200000;
                        m_PushStreamParam.iVideoMinBitRate = 200000;
                        m_PushStreamParam.iVideoMaxBitRate = 300000;
            ...
            if(iNetworkStatus == ENUM_PUSH_STREAM_NETWORK_STATUS_GOOD)
            {
                if(m_PushStreamParam.iVideoBitRate < 600000)
                {
                    m_PushStreamParam.iVideoBitRate = 600000;
                    m_PushStreamParam.iVideoMinBitRate = 600000;
                    m_PushStreamParam.iVideoMaxBitRate = 800000;
            ...
            {
                if(m_PushStreamParam.iVideoBitRate > 200000)
                {
                    m_PushStreamParam.iVideoBitRate = 200000;
                    m_PushStreamParam.iVideoMinBitRate = 200000;
                    m_PushStreamParam.iVideoMaxBitRate = 300000;
            ...
            int iVideoPktCnt = m_AVPacketQueue.GetStreamPacketCnt(m_iVideoStreamID);
            if(iVideoPktCnt >= m_PushStreamStatus.GetVideoCacheMax())
            {
                m_PushStreamStatus.AddVideoLostCnt();
            }
            else
            {
                // 编码
                ...
                if (m_pVideoEncode->Encode(pOutputFrame, &Packet))
                {
...
{
    int iVideoFrameRate = m_PushStreamParam.iVideoFrameRate;
    
    m_PushStreamStatus.SetVideoFrameRate(iVideoFrameRate);
    m_PushStreamStatus.Reset();
    
    
    // 是否丢包
    bool bLostVideoPacket = false;

    
    int64_t iLastCalcTime = av_gettime();    
	while (!m_bSendThreadExit)
	{
		try
		{
            int iSpacingTime = (int)((av_gettime() - iLastCalcTime) / 1000000);
            if(iSpacingTime >= m_PushStreamStatus.GetCalculatePeriod())
            {
                int iCurrStauts = m_PushStreamStatus.Calculate(iSpacingTime);
                double fVideoAvgFrameRate = m_PushStreamStatus.GetVideoAvgFrameRate();
                double fVideoAvgLostCnt = m_PushStreamStatus.GetVideoLostCnt();
                double fBitrate = m_PushStreamStatus.GetBitrate();
                int iReconnCnt = m_PushStreamStatus.GetReconnectCnt();
                
                if(nullptr != m_pNotify)
                {
                    m_pNotify->NetworkStauts(iCurrStauts,
                                             fVideoAvgFrameRate,
                                             fBitrate,
                                             fVideoAvgLostCnt,
                                             iReconnCnt,
                                             m_SendDeal.IsConnect());
                }
                iLastCalcTime = av_gettime();
            }
			...
			if (!m_SendDeal.IsConnect())
			{
                ...
                m_PushStreamStatus.AddReconnectCnt();
                
                if(m_SendDeal.IsConnect())
                {
                    if(nullptr != m_pNotify)
                    {
                        m_pNotify->Reconnected();
                    }
                }
			...
			if (Packet.stream_index == m_iVideoStreamID)
			{
                ...
                {
                    m_PushStreamStatus.AddVideoSend(Packet.size);
                }
			...
			else if (Packet.stream_index == m_iAudioStreamID)
			{
                ...
                {
                    m_PushStreamStatus.AddAudioSend(Packet.size);
                }
			}
